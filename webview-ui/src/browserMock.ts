...
async function decodePng(url: string): Promise<DecodedPng> {
  ...
  if (!res.ok) throw new Error(`Failed to fetch PNG: ${url} (${res.status.toString()})`);
...
export async function initBrowserMock(): Promise<void> {
  ...
  mockPayload = { characters, floorSprites, wallSets, furnitureCatalog: catalog, furnitureSprites, layout };
...
export function dispatchMockMessages(): void {
  ...
  const { characters, floorSprites, wallSets, furnitureCatalog, furnitureSprites, layout } = mockPayload;
